...
const GuideView: React.FC<Props> = ({ animationController }) => {
  ...
  return (
    ...
    >
      <Text
        testID="SplashGuideView_Title"
        accessibilityLabel="SplashGuideView_Title"
        style={styles.title}
      >
       {Values.Splash.guide}
      </Text>
      ...
      >
        {Values.Splash.guideDescription}
      </Animated.Text>
      <Animated.Image
        style={[styles.image, { transform: [{ translateX: imageAnim }] }]}
        source={AppImages.guide_image}
      />
    </Animated.View>
  );
};
